...
export const InputLogin = forwardRef<HTMLInputElement, InputLoginProps>(
  (
    ...
    return (
      //Div root 
      <div
        className={`flex flex-col justify-start relative gap-2`}
      >
        {/* Icone */}
        ...
        <input
          ...
          name={name}
          ref={ref}
          className={`peer block min-h-[auto] px-9 w-full border-b-2 ${hasError ? `border-light-red` : `border-primary`} 
                      bg-transparent py-[0.35rem] leading-[1.6] outline-none transition-all duration-200 ease-linear 
                      ...
                      dark:peer-focus:${hasError ? `text-light-red` : `text-primary`}`}
          id={inputId}
          {...props}
        />
